...
function StripStr($str){
	// get_magic_quotes_gpc() removed in PHP 5.4+, always returns false
...
function Val($name,$method='GET',$type=0,$isArray=0){
	...
	switch($type){
		case 0:
		case 'string':
			$value=($value !== null) ? (($isArray==0) ? StripStr($value) : array_map('StripStr',(array)$value)) : ($isArray==0 ? '' : array());
			break;
		case 1:
		case 'int':
...
function UBBToHTML($str=''){
	if(!empty($str)){
		...
		$str=preg_replace_callback("/\[code\](.*?)\[\/code]/is", function($matches) { return DoCode($matches[1]); }, $str);
		$str=str_replace("\n",'<br />',$str);
