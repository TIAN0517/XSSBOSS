...
/* 							$setkeyV=str_replace("%3a",':',$setkeyV);
	...
	case 'view':
		...
		$countSql = "SELECT COUNT(*) FROM ".Tb('project_content')." WHERE projectId='{$id}' {$where}";
		...
		$domainss=$db->Dataset("select distinct domain from ".Tb('project_content')." WHERE projectId='{$id}' {$where}");
		if($domainss){
