...
class Smarty_Compiler extends Smarty {
    ...
    /**#@-*/
    /**
     * The class constructor (PHP 8+ compatible).
     */
    function __construct()
    {
        // Call parent constructor (Smarty class)
        $this->Smarty();
    ...
    function _compile_file($resource_name, $source_content, &$compiled_content)
    {
        ...
        /* fetch all special blocks and replace them with placeholders - PHP 7+ compatible */
        // Match {php}...{/php} and {literal}...{/literal} blocks

        // First pass: match {php} blocks and save to _folded_blocks
        $php_pattern = "~{$ldq}\s*php\s*{$rdq}(.*?){$ldq}\s*/php\s*{$rdq}~s";
        preg_match_all($php_pattern, $source_content, $php_matches, PREG_SET_ORDER);

        // Second pass: match {literal} blocks and save to _folded_blocks
        $literal_pattern = "~{$ldq}\s*literal\s*{$rdq}(.*?){$ldq}\s*/literal\s*{$rdq}~s";
        preg_match_all($literal_pattern, $source_content, $literal_matches, PREG_SET_ORDER);

        // Combine all blocks: php first, then literal (for sequential processing)
        $this->_folded_blocks = array_merge($php_matches, $literal_matches);
        reset($this->_folded_blocks);

        // Replace {php} blocks with placeholder
        $block_index = 0;
        $source_content = preg_replace_callback($php_pattern, function($m) use (&$block_index) {
            return '{php:' . $block_index++ . '}';
        }, $source_content);

        // Replace {literal} blocks with placeholder
        $block_index = 0;
        $source_content = preg_replace_callback($literal_pattern, function($m) use (&$block_index) {
            return '{literal:' . $block_index++ . '}';
        }, $source_content);
    ...
    /**
     * Callback for replacing literal blocks - PHP 8+ compatible replacement for /e modifier
     * Wraps content in {php} tags as placeholders to be processed later
     *
     * @param array $m Regex match
     * @return string Replacement text
     */
    function _smarty_literal_replace($m)
    {
        // Determine which type of block matched
        if (isset($m[1]) && $m[1] !== '') {
            // {*comment*}: return empty (comments are removed)
            return '';
        } elseif (isset($m[2]) && $m[2] !== '') {
            // {literal}...{/literal}: wrap in {php} tags - content will be output later
            return '{php}' . $m[2] . '{/php}';
        } elseif (isset($m[3]) && $m[3] !== '') {
            // {php}...{/php}: wrap in {php} tags
            return '{php}' . $m[3] . '{/php}';
        }
        return '';
    }
    ...
    function _compile_tag($template_tag)
    {
        ...
        // Handle {php:N} and {literal:N} placeholders
        if (preg_match('/^(php|literal):(\d+)$/', $template_tag, $pmatch)) {
            $type = $pmatch[1];
            $index = (int)$pmatch[2];
            // Get the corresponding block from _folded_blocks
            $blocks = array_values($this->_folded_blocks);
            if (isset($blocks[$index])) {
                $block = $blocks[$index];
                if ($type === 'php') {
                    // PHP block - output as PHP code
                    return '<?php ' . trim($block[1]) . ' ?>';
                } else {
                    // Literal block - output content directly
                    return $block[1];
                }
            }
            return '';
